...
\begin{theorem}
    已知 $X$ 的分布函数为 $F(x)$, 则 
    \begin{flalign*}
        EX&=\int_{0}^{+\infty}[1-F(x)]\dd x-\int_{-\infty}^{0}F(x)\dd x\\ 
        EX^2&=2\qty[\int_{0}^{+\infty}x[1-F(x)]\dd x-\int_{-\infty}^{0}xF(x)\dd x].
    \end{flalign*}
    特别地, $X$ 为非负随机变量时, 则 
    \begin{flalign*}
...
\begin{solution}
    ...
    \begin{flalign*}
        F_X&=P(\sj{A}_2)P\qty{X\leqslant x|\sj{A}_2}+P\qty(\bar{\sj{A}}_2)P\qty{X\leqslant x|\sj{\bar{A}}_2}\\ 
            &=\dfrac{13}{30}\qty(1-\e ^{-10x})+\dfrac{17}{30}\qty(1-\e ^{-15x})
    \end{flalign*}
