...
\begin{solution}
    \begin{enumerate}[label=(\arabic*)]
        ...
        \item 由 $\displaystyle\lim _{x\to 0}\dfrac{1}{\displaystyle\prod\limits ^{n}_{k=1}\sqrt[k] {\cos kx}}=1$, 故 $\displaystyle1-\prod ^{n}_{k=1}\sqrt[k] {\cos kx}\sim -\sum ^{n}_{k=1}\dfrac{1}{k}\ln \cos kx\sim \sum ^{n}_{k=1}\frac{1}{k}\ln \left( 1-\cos kx\right)(x\to0)$,
              ...
              \end{flalign*}
        \item \textbf{法一: }因为 $\displaystyle\lim_{x\to0}\frac{n!x^n}{\displaystyle\prod\limits_{k=1}^{n}\sin kx}=1$, 所以 $\displaystyle n!x^n\sim\prod_{k=1}^{n}\sin kx~ (x\to0)$.
              \begin{flalign*}
...
\begin{example}
    设 $\cos x-1=x\sin\alpha(x)$, 其中 $|\alpha(x)|<\dfrac{\pi}{2}$, 则当 $x\to0$ 时, $\alpha(x)$ 是 (\quad).
    \begin{tasks}(2)
        \task 比 $x$ 高阶的无穷小量
        \task 比 $x$ 低阶的无穷小量
        \task 与 $x$ 同阶但不等价的无穷小量
...
\begin{example}
    设 $g(x)$ 可导, 且当 $x\to0$ 时,  $g(x)$ 是 $x$ 的高阶无穷小, 则当 $x\to0$  时, 必有 (\quad).
    \begin{tasks}(2)
        \task $g'(x)$ 是无穷小量
...
\begin{example}
    已知 $\displaystyle\lim_{n\to\infty}\dfrac{n^{\alpha}}{n^\beta-(n-1)^\beta}=2023$, 则 $\alpha,\beta$ 的值为 (\quad).
    \begin{tasks}(2)
...
\begin{solution}
    ...
    \end{flalign*}
    \textbf{法五: }由定理 \ref{fsimg}, 可得 $$
    \lim_{x\to+\infty}(x+1)\qty[\ln\qty(x^2+x)-2\ln(1+x)]=\lim_{x\to+\infty}\dfrac{x+1}{x^2+x}\qty(x^2+x)\qty[\ln\qty(x^2+x)-\ln(1+x)^2]=-\lim_{x\to+\infty}\dfrac{(x+1)^2}{x^2+x}=-1.
    $$
\end{solution}
...
\begin{example}
    ...
    排列起来, 使排列在后的是前一项的高阶无穷小量, 则正确的排列次序为 (\quad).
    \begin{tasks}(4)
