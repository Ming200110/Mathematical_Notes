...
\begin{solution}
    ...
    % 对于 D 选项, 易验证 $\vb*{A}^{-1}$ 与 $\vb*{A}^{*}$ 都是实对称矩阵, 且 $\vb*{A}^{-1}$ 与 $\vb*{A}^{*}$ 的特征值分别为 $\dfrac{1}{\lambda_i}$ 与 $\dfrac{|\vb*{A}|}{\lambda_i}$ 也全大于零, 故 $\vb*{A}$ 矩阵的逆矩阵 $\vb*{A}^{-1}$ 及伴随矩阵 $\vb*{A}^{*}$ 也为正定矩阵, D 正确.
...
\begin{definition}[矩阵合同]
    ...
    $$
    其中 $ \boldsymbol{C}^{\top} \boldsymbol{A} \boldsymbol{C}=\boldsymbol{B} $.\index{矩阵合同}
\end{definition}
