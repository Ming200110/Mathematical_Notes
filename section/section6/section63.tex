...
\begin{definition}[对面积的曲面积分 (第一型曲面积分)]
    ...
    $$\iint\limits_{\varSigma} f(x, y, z) \dd S = \lim _{\lambda \to 0} \sum_{i=1}^{n} f\left(x_{i}, y_{i}, z_{i}\right) \Delta S_{i}$$
...
\begin{solution}
    由题意 
    \begin{flalign*}
        u(x,y)&=\int 2xy \dd x=x^2y+\psi (y)\\ 
        u(x,y)&=\int \qty(x^2+2y)\dd y=x^2y+y^2+\varphi(x)
    \end{flalign*}
...
\begin{solution}
    ...
    \begin{flalign*}
        I & =\oiint\limits_{\varSigma+\varSigma_1}-\iint\limits_{\varSigma_1}=2\iiint\limits_\Omega(x+y+z)\dd V+\iint\limits_D\dd z\dd x=2\int_0^2x\dd x\iint\limits_{D_x}\dd y\dd z+2\int_1^2y\dd y\iint\limits_{D_y}\dd z\dd x+2\pi \\
          & =2\int_0^2x\cdot\dfrac{1}{2}\pi\cdot\sqrt{2x-x^2}\cdot\sqrt{2x-x^2}\dd x+2\int_1^2y\cdot \pi\cdot\sqrt{2y-y^2}\cdot\sqrt{2y-y^2}\dd y+2\pi                                                                                \\
          & =2\pi\int_{0}^{2}\qty(2x^2-x^3)\dd x+4\pi \int_1^2\qty(2y^2-y^3)\dd y+2\pi=\dfrac{8\pi }{3}+\dfrac{11\pi}{3}+2\pi=\dfrac{25\pi}{3}.
    \end{flalign*}
\end{solution}
...
\end{example}
\begin{solution}
    由 Gauss 公式得 $$\displaystyle I=\iiint\limits_\Omega(1+2+3)\dd V$$
    其中 $\Omega$ 为 $|x-y+z|+|y-z+x|+|z-x+y|\leqslant 1$,
...
\begin{theorem}[Euler 齐次函数]
    \index{Euler 齐次函数}若 $f$ 是 $n$ 元 $k$ 次齐次函数, 即 $f(tx_1, tx_2, \cdots , tx_n)=t^{k}f(x_1, x_2, \cdots ,x_n)$, 则有 
    $$
...
\begin{solution}
    ...
    \begin{flalign*}
        \oiint\limits_\varSigma f(x,y,z)\dd S&=\dfrac{1}{4}\oiint\limits_\varSigma\qty(x\pdv{f}{x}+y\pdv{f}{y}+z\pdv{f}{z})\dd S=\dfrac{1}{2}\oiint\limits_\varSigma\pdv{f}{x}\dd y\dd z+\pdv{f}{y}\dd z\dd x\pdv{f}{z}\dd x\dd y\\ 
        &=\dfrac{1}{2}\iiint\limits_\Omega\qty(\pdv[2]{f}{x}+\pdv[2]{f}{y}+\pdv[2]{f}{z})\dd V=\iiint\limits_\Omega\qty(11x^2-4y^2+17z^2)\dd V
    \end{flalign*}
...
\begin{theorem}[Stokes 积分法]
    \index{Stokes 积分法}设 $ \varSigma $ 是逐段光滑的单闭曲线 $ \Gamma $ 所包围的非封闭光滑双侧曲面, 取某定侧 $ \varSigma^{+} $, 按右手规则确定 $ \Gamma $ 的正向 $ \Gamma^{+}$, $\Omega $
...
\begin{theorem}[三重积分分部积分公式]
    \index{三重积分分部积分公式}该定理是定理 \ref{iintlimitsduvxxy} 推广到三维的形式, 设 $\Omega$ 是由一张或几张分片光滑的双侧封闭曲面围成的空间有界闭区域, 若 $u,v$ 在闭区域 $\Omega$ 上有连续的一阶偏导数, 则
    $$\begin{cases}
...
\begin{example}
    设 $u=u(x,y,z)$ 二阶连续可偏导, 且 $u$ 满足 $\text{div}(\vb{grad}~u)=\qty(x^2+y^2+z^2)^{\frac{3}{2}}$, 设 $\varSigma_r:x^2+y^2+z^2=r^2$, $\vb*{n}$ 为曲面 $\varSigma_r$ 上任一点外法向量的单位向量,
    \begin{enumerate}[label=(\arabic{*})]
        \item 当 $r=1$ 时, 求 $\displaystyle \oiint\limits_{\varSigma_1}\pdv{u}{\vb*{n}}\dd S$;
        \item 设 $f=f(x,y,z),~g=g(x,y,z)$ 在封闭空间区域 $\Omega$ 内具有连续的一阶偏导数, 证明: $$\displaystyle \iiint\limits_\Omega f\pdv{g}{x}\dd V=\oiint\limits_{\partial\Omega}fg \dd y \dd z- \iiint\limits_\Omega g\pdv{f}{x}\dd V$$
        \item 求 $\displaystyle \lim_{r \to 0^+}\displaystyle \dfrac{1}{r^7}\iiint\limits_{\Omega_r}\qty(\vb{grad}~r\cdot \vb{grad}~u)\dd V$.
    \end{enumerate}
\end{example}
\begin{solution}
    \begin{enumerate}[label=(\arabic{*})]
        \item 因为 $\displaystyle \mathrm{div}(\vb{grad}~u)=\pdv[2]{u}{x}+\pdv[2]{u}{y}+\pdv[2]{u}{z}=\qty(x^2+y^2+z^2)^{\frac{3}{2}}$, 所以 
        \begin{flalign*}
            \oiint\limits_{\varSigma_1}\pdv{u}{\vb*{n}}\dd S&=\oiint\limits_{\varSigma_1}\pdv{u}{x}\cos\alpha+\pdv{u}{y}\cos\beta+\pdv{u}{z}\cos\gamma \dd S=\iiint\limits_{\Omega_1}\qty(\pdv[2]{u}{x}+\pdv[2]{u}{y}+\pdv[2]{u}{z})\dd V\\ 
            &=\iiint\limits_{\Omega_1}\qty(x^2+y^2+z^2)^{\frac{3}{2}}\dd V=\int_{0}^{\pi}\dd \varphi\int_{0}^{2\pi}\dd \theta\int_{0}^{1}\rho^5\sin\varphi\dd \rho=\dfrac{2\pi}{3}.
        \end{flalign*}
        \item 由 Gauss 公式: $\displaystyle \oiint\limits_{\Sigma} P \dd  y \dd  z+Q \dd  z \dd  x+R \dd  x \dd  y =\iiint\limits_{\Omega}\left(\frac{\partial P}{\partial x}+\frac{\partial Q}{\partial y}+\frac{\partial R}{\partial z}\right) \mathrm{d} V$, 令 $P=Q=R=fg$, 则 
        \begin{flalign*}
            \oiint\limits_{\Sigma} fg( \dd  y \dd  z+ \dd  z \dd  x+ \dd  x \dd  y)=\iiint\limits_{\Omega}\qty(f_xg+fg_x+f_yg+fg_y+f_zg+fg_z)\dd V
        \end{flalign*}
        整理得: $\begin{cases}
            \displaystyle\iiint\limits_\Omega f\frac{\partial g}{\partial x}\dd V=\oiint\limits_{\partial \Omega}fg\dd y\dd z-\iiint\limits_\Omega g\frac{\partial f}{\partial x}\dd V, \\[6pt]
            \displaystyle\iiint\limits_\Omega f\frac{\partial g}{\partial y}\dd V=\oiint\limits_{\partial \Omega}fg\dd z\dd x-\iiint\limits_\Omega g\frac{\partial f}{\partial y}\dd V, \\[6pt]
            \displaystyle\iiint\limits_\Omega f\frac{\partial g}{\partial z}\dd V=\oiint\limits_{\partial \Omega}fg\dd x\dd y-\iiint\limits_\Omega g\frac{\partial f}{\partial z}\dd V.
        \end{cases}$ 其中 $\partial\Omega$ 表示空间 $\Omega$ 的边界.
        \item 当 $r\to0^+$ 时, $r=\sqrt{x^2+y^2+z^2}$, 那么
        \begin{flalign*}
            \iiint\limits_{\Omega_r}\qty(\vb{grad}~r\cdot \vb{grad}~u)\dd V&=\iiint\limits_{\Omega_r}\qty(\dfrac{x}{\sqrt{x^2+y^2+z^2}}\cdot\pdv{u}{x}+\dfrac{y}{\sqrt{x^2+y^2+z^2}}\cdot\pdv{u}{y}+\dfrac{z}{\sqrt{x^2+y^2+z^2}}\cdot\pdv{u}{z})\dd V\\ 
            &=\oiint\limits_{\partial \Omega_r}\sqrt{x^2+y^2+z^2}\qty(\pdv{u}{x}\dd y\dd z+\pdv{u}{y}\dd z\dd x+\pdv{u}{z}\dd x\dd y)\\ 
            &~~~~~-\iiint\limits_{\Omega_r}\sqrt{x^2+y^2+z^2}\qty(\pdv[2]{u}{x}+\pdv[2]{u}{y}+\pdv[2]{u}{z})\dd V\\ 
            &=r\iiint\limits_{\Omega_r}\qty(\pdv[2]{u}{x}+\pdv[2]{u}{y}+\pdv[2]{u}{z})\dd V-\iiint\limits_{\Omega_r}\qty(x^2+y^2+z^2)^2\dd V\\ 
            &=r\iiint\limits_{\Omega_r}\qty(x^2+y^2+z^2)^{\frac{3}{2}}\dd V-\int_{0}^{\pi}\dd \varphi\int_{0}^{2\pi}\dd\theta\int_{0}^{r}\rho^6\sin\varphi\dd \rho\\ 
            &=r\cdot2\cdot2\pi\cdot\dfrac{1}{6}r^6-2\cdot2\pi\cdot\dfrac{1}{7}r^7=\dfrac{2\pi}{21}r^7
        \end{flalign*}
        因此  $\displaystyle \lim_{r \to 0^+}\displaystyle \dfrac{1}{r^7}\iiint\limits_{\Omega_r}\qty(\vb{grad}~r\cdot \vb{grad}~u)\dd V=\dfrac{2\pi}{21}$.
    \end{enumerate}
\end{solution}
...
\begin{theorem}[第二型曲面积分的分部积分公式]
    \index{第二型曲面积分的分部积分公式}设光滑曲面 $\varSigma$ 的边界曲线 $\Gamma$ 按段光滑, 函数 $P_i(x,y,z),\\Q_i(x,y,z),~R_i(x,y,z)~~(i=1,2)$ 在 $\varSigma$ (连同 $\Gamma$) 上有连续的一阶偏导数, 则
    \begin{flalign*}
