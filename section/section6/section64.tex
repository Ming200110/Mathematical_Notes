...
\begin{definition}
    ...
    \begin{flalign*}
         ...
         & \vb{rot}~\vb*{A}:=\curl{\vb*{A}}:=\mqty|\vb*{i}                                                                    & \vb*{j} & \vb*{k} \\ \displaystyle\pdv{x}&\displaystyle\pdv{y}&\displaystyle\pdv{z}\\P&Q&R|.
    \end{flalign*}
\end{definition}
...
\begin{example}
  ...
  \begin{solution}
    ...
    \begin{flalign*}
      \div{u\vb*{A}}&=\pdv{uP}{x}+\pdv{uQ}{y}+\pdv{uR}{z}=u_xP+uP_x+u_yQ+uQ_y+u_zR+uR_z\\
      &=u(P_x+Q_y+R_z)+\vb*{A}\cdot (u_x,u_y,u_z)=u\div{\vb*{A}}+\vb*{A}\grad{u}
    \end{flalign*}
    ...
    \begin{flalign*}
      \curl{u\vb*{A}}&=\mqty|\vb*{i}& \vb*{j} & \vb*{k} \\ \displaystyle\pdv{x}&\displaystyle\pdv{y}&\displaystyle\pdv{z}\\uP&uQ&uR|=\qty(\pdv{uR}{y}-\pdv{uQ}{z})\vb*{i}+\qty(\pdv{uP}{z}-\pdv{uR}{x})\vb*{j}+\qty(\pdv{uQ}{x}-\pdv{uP}{y})\vb*{k}\\ 
      &=(u_yR+uR_y-u_zQ-uQ_z)\vb*{i}+(u_zP+uP_z-u_xR-uR_x)\vb*{j}+(u_xQ+uQ_x-u_yP-uP_y)\vb*{k} \\ 
      &= u\qty[(R_y-Q_z)\vb*{i}+(P_z-R_x)\vb*{j}+(Q_x-P_y)\vb*{k}]+(u_x,u_y,u_z)\times(P,Q,R)\\ 
      &=\curl{(u\vb*{A})}=u\curl{A}+\grad{u}\times \vb*{A}
    \end{flalign*}
    选项 D 正确.\\ 
